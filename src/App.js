...
const HF_ACCESS_TOKEN = process.env.REACT_APP_API_KEY;
...
function App() {
  ...
  const handleClick = async (e) => {
    ...
    if (input) {
      const inputs = `My ideal first date is taking a walk on the beach. My date's ideal first date is: ${input}. 3 great first date ideas include: `;
      const model = "gpt2";
      // Call the conversational method and handle the result
      setLoading(true);
      await inference
        .textGeneration({
          inputs,
        ...
        .then((result) => {
          setMessages([
            ...
            { type: "bot", text: result.generated_text },
          ]);
        })
        .catch((err) => {
          setLoading(false);
      ...
      {openChat && (
        <div className="animate-topslide absolute w-[300px] right-2 bottom-2 bg-gray-800 min-h-[350px]">
          ...
          <div>
            ...
            <form
              className="flex p-4 bg-white absolute bottom-0 border-t border-gray-300"
            ...
            >
              ...
              <button
                onClick={handleClick}
                className="px-4 py-2 font-bold text-white bg-blue-500 rounded hover:bg-blue-600 focus:outline-none focus:ring focus:ring-blue-200"
              >
                {loading ? (
